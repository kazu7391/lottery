...
class LanguageController extends Controller
{
    ...
    public function langStore(Request $request)
    {
        ...
        if ($request->hasFile('flag')) {
            $path = getFilePath('language');
            $size = getFileSize('language');
            try {
                $filename       = fileUploader($request->flag, $path, $size, $language->flag);
    ...
    public function langHide($id)
    {
        $lang = Language::find($id);
        $lang->is_hidden = 1;
        $lang->save();
        $notify[] = ['success', 'Language hidden successfully'];
        return back()->withNotify($notify);
    }
    ...
    public function langShow($id)
    {
        ...
        $lang->is_hidden = 0;
        $lang->save();
        $notify[] = ['success', 'Language shown  successfully'];
        return back()->withNotify($notify);
    }

<div class="dashboard-nav d-flex flex-wrap align-items-center justify-content-between bg--dark">
    ...
    <div class="nav-right d-flex flex-wrap align-items-center gap-3">
        ...
        @if ($general->multi_language)
            ...
            @endphp
            <select class="langSel form--control h-auto px-2 py-1 border-0 text-white" name="langSel">
                @foreach ($language as $item)
                    <option style="color: black;" @if (session('lang') == $item->code) selected @endif value="{{ $item->code }}">{{ __($item->name) }}</option>
                @endforeach
            </select>
        @endif
